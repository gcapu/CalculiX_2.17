...
extern char  readfbdFlag;                   /* 1: currently reading a command file */
...
void pre_move( char *record)
{
  ...
  if(anz->e>0)
  {
    // correct the elementsm needed if the mesh was mirrored
    setall=getSetNr("all");
...
#define PROJ_LOOPS 40
  ...
  if((functionFlag)&&(!sum_tri))
  {
    if((compare(action, "nor",3) == 3)&&(set[trgtNr].anz_sh))
    {
      for(i=0; i<set[trgtNr].anz_sh; i++)
      {
        if(shape[set[trgtNr].shp[i]].type==4) projSetToNurbs( shape[set[trgtNr].shp[i]].p[0], set, setNr, point);
        else
	{
	  Stmp=-1;
	  ...
	  if(Stmp>-1)
	  {
	    projSetToNurbs( Stmp, set, setNr, point);
          ...
          */
          ...
          do
          {
            for(j=0; j<curr_n_closest_tri; j++)
            {
              ...
              if(( dist != OUTSIDE )&&(abs(dist)<abs(mindist)))
              {
  ...
  else if( compare( action, "rot", 3) == 3)
  {
    ...
    */
    ...
    for (i=0; i<sum_pproj; i++)
    {
      ...
      pa1[0]=l;  /* Attention: =l is not a 1 ('one'), its an lowercase 'L' */
      pa1[1]=0.;
      pa1[2]=0.;
      pa2[0]=l;
      pa2[1]=1.;
    ...
    if(( compare( action, "nor", 3) == 3)&&(set[trgtNr].anz_sh))
    {
      ...
      for (i=0; i<set[trgtNr].anz_sh; i++)
      {
        if(shape[set[trgtNr].shp[i]].type==4)
          projSetToNurbs( shape[set[trgtNr].shp[i]].p[0], set, setNr, point);
      }
...
int zap(char *record)
{
  ...
  {
    /* check if a remaining surf uses this shape */
    /* go over all surfs in setNr and remove this surf from shape[].s */
    for ( i=0; i<set[setNr].anz_s; i++)
    {
      j=surf[set[setNr].surf[i]].sh;
      if(j>-1) shape[j].ns=iremove(&shape[j].s, shape[j].ns, set[setNr].surf[i]);
    } 
    if(getSetNr(specialset->tmp)>-1) delSet(specialset->tmp);
    setNrbuf=pre_seta(specialset->tmp, "i", 0 );    
    for ( i=0; i<set[setNr].anz_sh; i++)
    {
      /* do not delete if shape[].ns>0 */
      if(shape[set[setNr].shp[i]].ns>0) seta( setNrbuf, "h", set[setNr].shp[i]);
    }
    for ( i=0; i<set[setNrbuf].anz_sh; i++) setr( setNr, "h", set[setNrbuf].shp[i]);
    if(getSetNr(specialset->tmp)>-1) delSet(specialset->tmp);
...
int commandoInterpreter( char *type, char *string, int na, int nb, FILE *handle1, int addFlag, int *gtolFlag )
{
  ...
  double    lfbuf, pscal, x,y,z,dx,dy,dz, wx[2], wy[2];
  char      **substring=NULL;
  int       foundSubString;
  ...
  */
  ...
  else if (compareStrings(type, "DIST")>0)
  {
	...
	{
          j=set[setNr].node[i];
          x = dmax(x,node[node[j].nr].nx);
          dx = dmin(dx,node[node[j].nr].nx);
          y = dmax(y,node[node[j].nr].ny);
          dy = dmin(dy,node[node[j].nr].ny);
          z = dmax(z,node[node[j].nr].nz);
          dz = dmin(dz,node[node[j].nr].nz);
	}
        for( i=0; i<set[setNr].anz_p; i++ )
	{
          j=set[setNr].pnt[i];
      ...
      dz=(z-dz)* scale->w;
      printf(" dxyz: %f %f %f\n", dx,dy,dz);
      sprintf(parameter[0],"%f",dx);
      sprintf(parameter[1],"%f",dy);
      sprintf(parameter[2],"%f",dz);
      write2stack( 3, parameter);
    }
  ...
  else if (compareStrings(type, "HCPY")>0)
  {
    ...
    /* actual hcpy nr */
    /*
    sprintf(buffer,"!hcpy_psNr %d", psNr);
    pre_value(buffer);
    sprintf(buffer,"!hcpy_tgaNr %d", tgaNr);
    pre_value(buffer);
    sprintf(buffer,"!hcpy_gifNr %d", gifNr);
    pre_value(buffer);
    sprintf(buffer,"!hcpy_pngNr %d", pngNr);
    pre_value(buffer);
    */
  }
  ...
  else if (compareStrings(type, "THRS")>0)
  {
    ...
    if(comm[0]=='t')
    {
      ...
      strcpy(setname2, "+grp");
      ibuf=separateMeshes(specialset->thrs, setname2);
  ...
  else if (compareStrings(type, "CMAP")>0)
  {
    ...
    for (i=0; i<cmaps; i++)
    {
      if( compare( cmap_names[i], buffer, 1)==1)
      {
...
void pre_read( char *record )
{
  ...
  else if((compare( ext, "frd", 3)== 3)||(compare( param1, "ng", 2)== 2)||(compare( ext, "stl", 3)== 3)||(compare( ext, "inp", 3)== 3)||(compare( param1, "inp", 3)== 3)||(compare( param1, "foam", 4)== 4)) 
  {
    ...
    if(compare( ext, "frd", 3)== 3)
    {
      ...
      else { if(readfrd( datin, apre, &npre, &epre, &lpre, read_mode) <0) return; }
    }
    ...
    else
    {
      ...
      if(apre->nmax>anz->nmax) anz->nmax=apre->nmax;
      if(anz->nmin==MAX_INTEGER) anz->nmin=apre->nmin;
      ...
      if( (compare( param1, "add", 3)== 3)||(compare( param2, "add", 3)== 3))
      {
       for(i=0; i<apre->n; i++) 
       {
        ...
        node[node[n].nr].nx=(npre[npre[i].nr].nx-scale->x)/scale->w;
        node[node[n].nr].ny=(npre[npre[i].nr].ny-scale->y)/scale->w;
        node[node[n].nr].nz=(npre[npre[i].nr].nz-scale->z)/scale->w;
...
void pre_write( char *record )
{
  ...
  else if (compare( format, "fbd", 3)== 3)
  {
    ...
    descalPoints( anzGeo->p, point, scale);
    writefbd( setname, anz, anzGeo, point, line, lcmb, surf, body, nurbs, set, shape, value, format );
    scalPoints( anzGeo->p, point, scale);
  }
