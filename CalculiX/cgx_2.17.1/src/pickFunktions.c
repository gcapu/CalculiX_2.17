  ...
  */
  else if (compare(pickfunc, "qmsh", 4) == 4)
  {
    ...
    if ( type[0] == 'b') /* break (split) the pre-selected surface */
    {
      ...
      i=splitSurf( set[qmshbuf].line[set[qmshbuf].anz_l-1], entitybuf );
      if(i<0) { printf(" ERROR:%d: could not split the selected surface\n",i); }
      else
      {
        /* mesh again */
        seta(qmshbuf,"s",i);
  ...
  else if (compare(pickfunc, "qseq", 4) == 4)
  {
    ...
    {
      printf (" type mode:%c type:%c %c\n",mode[0],type[0],type[1]);
      if(set[setNrbuf].anz_l==0) printf (" ERROR: No line selected\n");
      else if(type[1]==' ')
      {
        for (j=0; j<set[setNrbuf].anz_l; j++ ) convertLine( set[setNrbuf].line[j], pickbuf);
      }
      else                               /* we need an lcmb */
      {
        ...
        for (j=0; j<anzGeo->c; j++ )
        {
          if((lcmb[j].nl==set[setNrbuf].anz_l)&&(lcmb[j].name != (char *)NULL))   /* same amount of lines */
	  {
            if (printFlag) printf ("check lcmb:%s \n", lcmb[j].name);
...
void movePoint(GLuint *picbuf, int x, int y)
{
  ...
  else if (surfNr>-1)
  {
    ...
    {
      projSetToNurbs(shape[surf[surfNr].sh].p[0],set,setNrbuf,point);
      movedp=-1;
  ...
  {
    /* project p to surf */
    projSetToNurbs(nurbsNr,set,setNrbuf,point);
    movedp=-1;
...
int splitSurf( int lmaster, int smaster )
{
  ...
  if(surf[smaster].sh>-1)
  {
    ...
    if(shape[surf[smaster].sh].type==4) projSetToNurbs(shape[surf[smaster].sh].p[0], set, line[lmaster].trk, point);
    else
    {
      s= surfToNurs(smaster);
      if(s>-1)
      {
        projSetToNurbs( s, set, line[lmaster].trk, point);
        delNurs( 1, &s );
      }
    }
    repLine(lmaster);
...
{
  int j,k,n1,n2,nn1,nn2,anz_n;
  int   nvalid[CUTS],nnew[CUTS];
  ...
  cutParam->anz_n=anz_n=0;  // must always be lower than the 1000 alloc elements 

  //printf("e:%d t:%d ptr:%x\n", elnr,e_enqire[elnr].type,cutParam);
  if (e_enqire[elnr].type == 1)  /* HEXA8 */
  {
    for(n1=0; n1<4; n1++)
    {
      if(n1==3) n2=0;
      ...
      g = AsplitL_l( n1pn, eu, ev, eg );
      ns[3] = g/v_betrag(n1n2);
      if(ns[3]>1.000001) continue;
      if(ns[3]<-0.000001) continue;
      v_scal( &g, eg, n1ns );
  ...
  else if (e_enqire[elnr].type == 4)  /* HEXA20 */
  {
    for(nn1=0; nn1<8; nn1++)
    {
      if(nn1==7) nn2=0;
  ...
  else if (e_enqire[elnr].type == 6)  /* TET10 */
  {
    for(nn1=0; nn1<6; nn1++)
    {
      if(nn1==5) nn2=0;
  ...
  if(anz_n>0)
  {
    ...
    {
      cg[0]+=cutParam->npre[nnew[j]][0];
      cg[1]+=cutParam->npre[nnew[j]][1];
      cg[2]+=cutParam->npre[nnew[j]][2];
    }
    cg[0]/=cutParam->anz_n;
    cg[1]/=cutParam->anz_n;
    cg[2]/=cutParam->anz_n;
    ...
    v_result( cg, &cutParam->npre[nnew[0]][0],  v01);
    rsort[0].r=0.;
    rsort[0].i=nnew[0];
    k=1;
    for(j=1; j<cutParam->anz_n; j++)
    {
      v_result( cg, &cutParam->npre[nnew[j]][0],  v02);
...
{
  int i,j,e,nn,nset,eset,anz_e,anz_nset,nel[3];
  static double p[3][3];
  double v02[3], v01[3], en[3], vn[3], pn[3], eu[3], ev[3];
  ...
  if ((qcutCounter == 3)||(flag=='v'))
  {
    ...
    for(e=1; e<anz_e; e++)
    {
      ...
      if(cutParam[i].anz_n>0)
      {
        /* generiere neue nodes */
        for(j=0; j<cutParam[i].anz_n; j++)
        {
          nn=nod( anz, &node, 0, anz->nnext++, cutParam[i].npre[cutParam[i].nnew[j]][0], cutParam[i].npre[cutParam[i].nnew[j]][1], cutParam[i].npre[cutParam[i].nnew[j]][2], 0 );
	  ...
	  //printf("qcut_nod:%d nod:%d\n",anz_nset, cutParam[i].nnew[j]); 
          ...
          qcut_nod[anz_nset].n1=cutParam[i].npre[cutParam[i].nnew[j]][4];
          qcut_nod[anz_nset].n2=cutParam[i].npre[cutParam[i].nnew[j]][5];
          qcut_nod[anz_nset].val=cutParam[i].npre[cutParam[i].nnew[j]][3];
          // re-use of nnew
